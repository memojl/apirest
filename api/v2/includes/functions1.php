...
global $page_url,$conec,$table,$tab_signup;
    ...
    if($json){
        ...
        if ($type == 'images') {
            //$revisar = getimagesize($filec);
	        //compruebo si las características del archivo son las que deseo 
	        if (!in_array($fileExtension, $validExt) && ($tamano_archivo < ($limite_file * 1024 * 1024))) {
		        $class = 'alert-danger';
		        $status = 'Error';
		        $msg = 'La imagen NO ha sido aceptada.';
	        } else {
                //Compruebo conde se va guardar en el servidor o en DB
                if($saveDB == 1 || $saveDB == 2){
                    $fileContenido = addslashes(file_get_contents($filec));
                    //Insertar imagen en la base de datos
                    $insertar = $conec->query("INSERT INTO upload_files (nombre, filec, created_at) VALUES ('$nombre_archivo', '$fileContenido', now())");
                    ...
                    if($insertar){
                        ...
                        if($numRows > 0){
                            ...
                            $imgUrl = 'data:image/jpeg;base64,' . base64_encode($row['filec']);//$row['ID'];
                        }
                    }else{
                        $class = 'alert-danger';
                        $status = 'Error';
                        $msg = 'Ocurrio algun error al guardar la Imagen. Intentelo nuevamente.';
                    } 
                }
                if($saveDB == '' || $saveDB == 2){
	        ...
	        //compruebo si las características del archivo son las que deseo 
	        if (!((strpos($tipo_archivo, $type)) && ($tamano_archivo < ($limite_file * 1024 * 1024)))) {
		        $class = 'alert-danger';
		        $status = 'Error';
